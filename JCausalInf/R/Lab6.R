...
")
  ...
  paste("a) Fit a 2-stage least square (2SLS) regression and estimate the causal effect of ‘educ’ on
‘lwage’.")
  ...
  paste("b) Obtain the corrected residuals using the coefficients from stage 2.")

  res.correct = Y - cbind(1, A, X)%*% coef(Y_hat)
  Y_hat$residuals = as.vector (res.correct )

  paste("c)  Using the corrected residuals, find the standard error of the 2SLS estimator.")

  SE<- sqrt(hccm(Y_hat, type = "hc0")[2,2])

  paste("d)  Find the 95% confidence interval for the 2SLS estimator.
")
