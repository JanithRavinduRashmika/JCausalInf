Lab2<- function() {
  paste("Q1)  Consider a completely randomized experiment (CRE) with a binary treatment A where n = 5 and
n1 = 3.")

  paste("1a) How many different treatment assignment mechanisms are possible?")

  choose(5,3)

  paste("1b) Write an R function to generate all possible treatment assignment mechanisms for any given
n and n1. (Hint: Use the functions choose and combn)
")
  ...
  paste("1c) Find all the possible treatment assignment mechanisms for a CRE with n = 5 and n1 = 3")

  trtassignment(5,3)



  paste("Q2) Angrist et al. (2009) conducted an experiment to evaluate different strategies to improve academic
...
gender (encoded by female) and baseline GPA(encoded by gpa0).")
  ...
  paste("2a)  Extract a subset of the dataset which only contains records for the control group (encoded
...
")
  ...
  paste("2b) Impute the missing outcomes with the observed average.")
  ...
  paste("2c)  Check the balance of the two covariates. (Hint: For each covariate, conduct a suitable twosample test to compare the covariate means or proportions in the control and treatment
groups)")
  ...
  paste("2d) Estimate the ATE without adjusting for covariates. (Hint: The unadjusted estimator is
numerically identical to the difference-in-means of the outcome)")
  ...
  paste("2e) Estimate the ATE adjusted for the covariates.")
