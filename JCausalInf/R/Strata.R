Strata <- function() {
  paste("Use the dataset homocyst in the package senstrat. The outcome is homocysteine, the homocysteine level, and the treatment is z, where z = 1 for a daily smoker and z = 0 for a never smoker.
...
st is a stratum indicator, defined by all the combinations of the discrete covariates.")
  paste("a) How many strata have only treated or control units?")
  ...
  paste("b) What is the proportion of the units in these strata?")

  sum(no_tc$no_t+no_tc$no_c)/nrow(data)

  paste("c) Drop these strata and perform a stratified analysis of the observational study. Report the
point estimator, variance estimator, and 95% confidence interval for the average causal effect.")
  ...
  paste("d) Run the OLS of the outcome on the treatment indicator and covariates without interactions.
Report the coefficient of the treatment and the standard error.")
  ...
  sqrt(hccm(fit2,type = "hc2")[2,2])


  paste("e)  If you do not drop the strata with only treated or control units, what will happen?")
