PropensityScore <- function() {
  paste("‘nhanes bmi.csv’ contains data from NHANES 2007–2008 survey to study whether participation
in school meal programs led to an increase in BMI for school children. The dataset has the following
covariates:")

  paste("a) Identify the outcome and the treatment variables")
  ...
  paste("b) Estimate the propensity scores using all the covariates.")

  pscore <- glm(a ~ x, family = binomial)$fitted.values

  paste("c) Discretize the propensity score by taking K = 5.")

  K<-5
  q<- quantile(pscore,(1:(K-1))/K)
  psq<-cut(pscore,breaks = c(0,q,1),labels = 1:K)

  paste("d) Based on propensity score stratification with K = 5, calculate the point estimate for average
causal effect (τ ) and its standard error.")
  paste("e) Construct an R function to estimate τ and its standard error based on propensity score stratification for any number of K strata. [Hint: Inputs for the function will be the outcome
variable, treatment variable and the vector of stratification.]")
  ...
  paste("f) Repeat part (d) for K = 10, 20, 50 and 80. Comment on your results.")
  ...
  paste("g)  Compute the difference in means estimator and outcome regression estimators with and without interaction terms together with their standard errors.
")
  ...
  paste("h) Compute the Horvitz–Thompson (HT) estimator and the H´ajek estimator with truncations
